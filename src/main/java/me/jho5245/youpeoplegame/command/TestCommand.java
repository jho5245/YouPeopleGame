...

{
	...
	public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args)
	{
		if (sender instanceof Player player)
		{
			...
			List<CustomMaterial> list = new ArrayList<>(CustomMaterial.values().stream()
					.filter(customMaterial -> customMaterial.getKey().getNamespace().equals(YouPeopleGame.getPlugin().getName().toLowerCase())).toList());
